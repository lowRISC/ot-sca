...
def plot_fvsr_stats(traces, leakage):
    ...
    plot.save_plot_to_file(
        np.stack((avg_fixed, avg_random), axis=0),
        np.array((1, 0)),
        2,
        "tmp/avg_fixed_random_traces.html",
    )
    plot.save_plot_to_file(diff_avg, None, 1, "tmp/diff_avg_trace.html")
    plot.save_plot_to_file(
        np.stack((var_fixed, var_random), axis=0),
        np.array((1, 0)),
        2,
        "tmp/var_fixed_random_traces.html",
    )
    plot.save_plot_to_file(diff_var, None, 1, "tmp/diff_var_trace.html")
...
def tvla_plotting_fnc(
    ...
    for i_order in range(num_orders):
        ...
        if trigger_high > 0:
            axs[1 + i_order].plot(
                xaxs[:trigger_high],
                ttest_trace[i_order, i_rnd, i_byte][:trigger_high],
                "grey",
            )
        if trigger_low > trigger_high:
            axs[1 + i_order].plot(
                xaxs[trigger_high:trigger_low],
                ttest_trace[i_order, i_rnd, i_byte][trigger_high:trigger_low],
                "k",
            )
        if trigger_low < num_samples:
            axs[1 + i_order].plot(
                xaxs[trigger_low:],
                ttest_trace[i_order, i_rnd, i_byte][trigger_low:],
                "grey",
            )
...
def run_tvla(ctx: typer.Context):
    ...
    if (
        ...
        for i_step in range(num_steps):
            ...
            if cfg["trace_file"] is None:
                ...
                if (
                    (save_to_disk_trace is True or save_to_disk_ttest is True)
                    and general_test
                    and i_step == 0
                ):
            ...
            if specific_test_byte:
                ...
                histograms = Parallel(n_jobs=num_jobs)(
                    delayed(compute_histograms_aes_byte)(
                        trace_resolution,
                        rnd_list,
                        byte_list,
                ...
                histograms = Parallel(n_jobs=num_jobs)(
                    delayed(compute_histograms_aes_bit)(
                        trace_resolution,
                        rnd_list,
                        bit_list,
                        traces[:, i: i + sample_step_hist],
    ...
    if cfg["plot_figures"]:
        ...
        if num_steps > 1:
            ...
            log.info(
                "Plotting T-test Statistics vs. Number of Traces, this may take a while."
            )
            ...
            for i_rnd in range(num_rnds):
                ...
                if specific_test:
                    ...
                    range_specified = (
                        True
                        if (
                            ("sample_start" in cfg and cfg["sample_start"] is not None)
                            or ("num_samples" in cfg and cfg["num_samples"] is not None)
                        )
                        else False
                    )
