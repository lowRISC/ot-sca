...
from target.communication.sca_edn_commands import OTEDN
...
"""EDN SCA capture script.

Captures power traces during different EDN operations.

Typical usage:
>>> ./capture_edn.py -c configs/edn_sca_cw310.yaml -p projects/edn_sca_capture
"""
...
def establish_communication(target, capture_cfg: CaptureConfig):
    ...
    Returns:
        ot_edn: The communication interface to the EDN SCA application.
        ot_prng: The communication interface to the PRNG SCA application.
        ot_trig: The communication interface to the SCA trigger.
    """
    # Create communication interface to OT EDN.
    ot_edn = OTEDN(target=target, protocol=capture_cfg.protocol)
    ...
    return ot_edn, ot_prng, ot_trig


def generate_data(test_mode, num_segments):
    """ Returns seeds used by the test.

    Either a fixed set of seeds or random seeds are generated.

    Returns:
        init_seed: The init seed.
        reseed: The reseed
    """
    seed_fixed = [0x73bec010, 0x9262474c, 0x16a30f76, 0x531b51de,
    ...
    for i in range(num_segments):
        ...
        if "fvsr" in test_mode:
            if sample_fixed:
                init_seed = seed_fixed
                reseed = seed_fixed
            else:
                for k in range(0, 12):
                    init_seed.append(random.getrandbits(32))
                for k in range(0, 12):
                    reseed.append(random.getrandbits(32))
            ...
            init_seed = seed_fixed
            reseed = seed_fixed
    return init_seed, reseed


def capture(scope: Scope, ot_edn: OTEDN, ot_prng: OTPRNG,
            capture_cfg: CaptureConfig, project: SCAProject, target: Target):
    """ Capture power consumption during execution of EDN SCA penetration tests.
    ...
    * edn.sca.bus_data_batch_fvsr
    ...
    with tqdm(total=remaining_num_traces, desc="Capturing", ncols=80, unit=" traces") as pbar:
        while remaining_num_traces > 0:
            ...
            init_seed, reseed = generate_data(capture_cfg.test_mode, num_segments)
            # Start the test based on the mode.
            if "batch" in capture_cfg.test_mode:
                if "fvsr" in capture_cfg.test_mode:
                    # In FvsR batch, the fixed seed and the number of segments
                    # is transferred to the device. The rest of the dataset is
                    # generated on the device. Trigger is set number of segments.
                    ot_edn.start_test(capture_cfg.test_mode, init_seed, reseed,
                    ...
                    ot_edn.start_test(capture_cfg.test_mode,
                                      capture_cfg.num_segments)
            else:
            ...
            response = ot_edn.edn_sca_read_response()
            # Check response. 0 for non-batch and the last data element in
            # batch mode.
            if "batch" in capture_cfg.test_mode:
                assert response == reseed[-1]
            else:
                assert response == 0
            ...
            for i in range(num_segments):
                ...
                # Store trace into database. Currently,
                # we do not store any other information that
                # the trace.
...
def main(argv=None):
    ...
    ot_edn, ot_prng, ot_trig = establish_communication(target, capture_cfg)

    # Configure SW trigger.
    ot_trig.select_trigger(1)

    # Capture traces.
    capture(scope, ot_edn, ot_prng, capture_cfg, project, target)
