...
class ChipShouterEMFI:
    def __init__(self, x_position_min: float, x_position_max: float,
                 ...
                 chipshouter_port: str, husky_sn: str):
        ...
        if self.pulse_width_min < min_ns:
            raise RuntimeError("Min pulse width shorter than supported (" + str(min_ns) + " ns)")
        # Check pulse step.
        if (self.pulse_width_min != self.pulse_width_max) and (self.pulse_width_step % min_ns != 0):
            raise RuntimeError("Only a pulse step width of " + str(min_ns) + " ns is supported")

        # Check trigger delay min.
        if self.trigger_delay_min < min_ns:
            raise RuntimeError("Min trigger delay shorter than supported (" + str(min_ns) + " ns)")

        # Check trigger delay step.
        if (self.trigger_delay_min != self.trigger_delay_max) and (self.trigger_step % min_ns != 0):
            raise RuntimeError("Only a trigger delay step  of " + str(min_ns) + " ns is supported")
    ...
    def arm_trigger(self, fault_parameters: dict) -> None:
        ...
        if (self.pos_y != fault_parameters['y_pos'] or
            ...
            self.pos_x = fault_parameters['x_pos']
            self.shv.move(fault_parameters['x_pos'],
                          fault_parameters['y_pos'], self.z_position)
        ...
        self.cwh.glitch.ext_offset = int(fault_parameters["trigger_delay"] *
                                         1e-9 / (1 / self.cwh.clock.clkgen_freq))
    ...
    def generate_fi_parameters(self) -> dict:
        ...
        parameters["glitch_voltage"] = random_float_range(self.voltage_min,
                                                          self.voltage_max,
                                                          self.voltage_step)
        parameters["glitch_width"] = random_float_range(self.pulse_width_min,
