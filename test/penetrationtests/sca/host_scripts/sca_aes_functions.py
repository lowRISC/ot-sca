...
def char_aes_single_encrypt(opentitantool, iterations, fpga, masking, key, text):
    target = DUT()
...
def char_aes_batch_fvsr_key(
    opentitantool, iterations, num_segments, fpga, masking, key
):
    target = DUT()
    chip = Chip(opentitantool)
    chip.reset_target()
...
def char_aes_batch_random(opentitantool, iterations, num_segments, fpga, masking, key):
    target = DUT()
    chip = Chip(opentitantool)
    chip.reset_target()
    # Clear the output from the reset
