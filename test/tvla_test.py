...
def ttest_compare_results(expected, received, delta) -> bool:
    ...
    numbers_match = np.all(
        np.logical_or(np.isnan(expected),
                      np.abs(expected - received) < delta))
...
def test_general_kmac_nonleaking_project():
    ...
    delta = 0.001
    assert ttest_compare_results(
        expected_trace, received_trace, delta
    ), f"{tvla} generated ttest_step values that don't match the expected ones"


def test_general_aes_nonleaking_project():
    project_path = TestDataPath(
        "tvla_general/ci_opentitan_simple_aes_fvsr.cwp")
    tvla = TvlaCmd(
        Args([
            "--project-file",
            str(project_path),
...
def test_general_leaking_histogram():
    ...
    assert ttest_significant(
        np.load("tmp/ttest.npy")
