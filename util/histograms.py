...
def compute_histograms_general(trace_resolution, traces, leakage):
    ...
    histograms = np.zeros(
        (num_rnds, num_bytes, num_leakages, num_samples, trace_resolution),
        dtype=np.uint32,
    )
        ...
        for i_byte in range(num_bytes):
            for i_sample in range(num_samples):
                histograms[i_rnd, i_byte, :, i_sample, :] = np.histogram2d(
                    leakage[rnd_list[i_rnd], byte_list[i_byte], :],
                    traces[:, i_sample],
                    bins=[
                        np.append(range(num_leakages), 9),
...
def compute_histograms_aes_bit(trace_resolution, rnd_list, bit_list, traces, leakage):
    ...
    num_samples = traces.shape[1]
    histograms = np.zeros(
        (num_rnds, num_bits, num_leakages, num_samples, trace_resolution),
        dtype=np.uint32,
    )
